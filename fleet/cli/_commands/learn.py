"""``fleet-cli learn`` — the learnability affordance.
...
from fleet.cli._output import emit_result

_TEXT = """\
fleet-cli — a clonable template for AgentCulture mesh agents.
...
Commands
--------
  fleet-cli whoami             Identity from culture.yaml.
  fleet-cli learn              This self-teaching prompt.
  fleet-cli explain <path>...  Markdown docs for any noun/verb path.
  fleet-cli overview           Descriptive snapshot of the agent.
  fleet-cli doctor             Check the agent-identity invariants.
  fleet-cli cli overview       Describe the CLI surface itself.
...
def _as_json_payload() -> dict[str, object]:
    return {
        ...
        "json_support": True,
        "explain_pointer": "fleet-cli explain <path>",
    }
