...
fleet-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. fleet-cli
...
#   - consumer-identifying prose: `guildmaster` → `fleet-cli` (NOT
...
If a re-sync would lose a fleet-cli adaptation, lift the change
...
#     → `which fleet-cli vendors from guildmaster`
