...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope fleet-cli
...
scope:** the default personal scope is **private** (`--scope fleet-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `fleet-cli`). Private records are served only to a recall in the same
